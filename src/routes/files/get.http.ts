...
const route: HTTP<{
	...
	prehandler: async (request, response, done) => {
		if (!request.headers.cookie || !request.headers.cookie.startsWith("token=")) { // Checks if the client has correctly provided a token in their request
		...
		const connection = await authenticate(token);
		...
		const accounts: { id: string, permissions: number, version: string }[] = await queryAsync("SELECT permissions, version FROM accounts WHERE id = ?", user.userId);
		...
		(request as any).path = join(process.env.SERVER_PATH!, request.query.path);
	},
