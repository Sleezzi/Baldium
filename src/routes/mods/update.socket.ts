...
const route: Socket = async (client, args: string, reply) => {
	try {
		...
		if (detailsFiltredByModLoader.length === 0) {
			await Logs(client.userId, "The client attempted to update a mod, but the mod does not have a version compatible with the modloader on Modrinth.", client.ip);
		...
		if (detailsFiltredByModLoaderAndMinecraftVersion.length === 0) {
			await Logs(client.userId, "The client attempted to update a mod, but the mod does not have a version compatible with the version of Minecraft on Modrinth.", client.ip);
		...
		if (latest.name === mod.name) {
			await Logs(client.userId, "The client attempted to update a mod, but it is already up to date", client.ip);
			reply(425, "The mod is already up to date");
			return;
		}
		if (await fsExist(`${process.env.SERVER_PATH}/mods/${mod.name}`)) {
		...
		for (const file of latest.files) {
			...
			.catch(async () => {
				await Logs(client.userId, "The client attempted to update a mod, but but an error has occurred", client.ip);
		...
		await Logs(client.userId, `The client updated the mod ${args} with the name "${mod.name}"`, client.ip);
