...
const route: HTTP<{
	...
	prehandler: async (request, response) => {
		if (!request.headers.cookie) { // Checks if the client has correctly provided a token in their request
			await Logs(null, "The client attempted to write server.properties but did not provide a valid token", request.clientIP);
		...
		if (!checkPermission("manage_files", accounts[0].permissions)) {
			await Logs(user, "The client attempted to write server.properties but their account does not have the necessary permissions", request.clientIP);
			return response.status(403).send({
		...
		const option = options.find((prop) => request.query.option === prop);
		if (!option) {
			await Logs(user, "The client attempted to write server.properties but the option is invalid in the request", request.clientIP);
			return response.status(401).send({
	...
	handler: async (request, response) => {
		try {
			...
			for (const line of lines) {
				const [option] = line.split("=") as any;
				if (option === request.option) {
					result.push(`${option}=${request.body}`);
					continue;
				}
				result.push(line);
			}
