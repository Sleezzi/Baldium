...
import { isNotTraversal, blocklist } from "../../components/files/Unauthorized";
...
const route: HTTP<{
	...
	prehandler: async (request, response, done) => {
		...
		if (!checkPermission("read_files", accounts[0].permissions)) {
			await Logs(user, "The client attempted to read a file, but their account does not have the necessary permissions", request.clientIP);
			return response.status(403).send({
		...
		if (!path) {
			await Logs(user, "The client attempted to read a file but did not provide a valid path to the file\n /!\\ The path was actually a hidden path", request.clientIP);
